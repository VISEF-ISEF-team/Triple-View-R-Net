...
def train(model, loader, optimizer, loss_fn, device=torch.device("cuda")):
    model.train()
    loop = tqdm(loader)
    epoch_loss = 0.0
    epoch_accuracy = 0.0
...
def main():
    ...
    num_epochs = 25
    checkpoint_path = "./files/original_attention_checkpoint.pth.tar"
    ...
    """Test model output"""
    # r = model(torch.rand(3, 256, 256).to(device), torch.rand(
    #     3, 256, 256).to(device), torch.rand(3, 256, 256).to(device))
    r = model(torch.rand(4, 1, 256, 256).to(device))
    ...
    df.to_csv("./files/original_attention_metrics_record.csv")
