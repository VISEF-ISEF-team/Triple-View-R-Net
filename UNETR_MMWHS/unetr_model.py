...
class UNETR_2D(nn.Module):
    def __init__(self, cf, outc=8):
        ...
        self.positions = torch.arange(
            start=0, end=cf["num_patches"], step=1, dtype=torch.int32).to(device=torch.device("cuda"))
    ...
    def forward(self, inputs):
        ...
        z0 = inputs.view(
            (batch, self.cf["num_channels"], self.cf["image_size"], self.cf["image_size"]))

        shape = (batch, self.cf["hidden_dim"],
                 self.cf["patch_size"], self.cf["patch_size"])
        z3 = z3.view(shape)
        z6 = z6.view(shape)
        z9 = z9.view(shape)
        z12 = z12.view(shape)
