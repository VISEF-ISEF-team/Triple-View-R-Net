...
def main():
    ...
    num_epochs = 100
    checkpoint_path = "./files/2D_unetr_focal-loss_checkpoint.pth.tar"
    ...
    loss_fn = DiceFocalLoss(to_onehot_y=True, softmax=True, gamma=4.0)

    # optimizer and scheduler
    optimizer = torch.optim.Adam(params=model.parameters(), lr=lr)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer=optimizer, mode="min", factor=0.2, patience=3)

    # scaler for mixed precision
    scaler = torch.cuda.amp.GradScaler()

    """Test model output"""
    r = torch.rand(4, 256, 256).to(device=torch.device("cuda"))
    print(f"Testing model output: {model(r).shape}")
    ...
    df.to_csv("./files/2D_unetr_focal-loss_metrics_record.csv")
