...
class YellowBlock(nn.Module):
    def __init__(self, in_channels, out_channels, normalization,
        ...
        self.conv_block = nn.Sequential(nn.Conv3d(in_channels, out_channels,
                                        ...
                                        nn.LeakyReLU(
                                            negative_slope=.01, inplace=True),
...
class UneTR(nn.Module):
    ...
    def forward(self, x):
        ...
        # arrange z values
        def arranger(z_emb): return rearrange(z_emb, 'b (x y z) d -> b d x y z',
...
if __name__ == '__main__':
    ...
    r = nn(torch.rand(2, 1, 128, 128, 16))
    print(f"Output shape: {r.shape}")

    mask = torch.rand(2, 1, 128, 128, 16)
    loss = DiceLoss()(r, mask)
