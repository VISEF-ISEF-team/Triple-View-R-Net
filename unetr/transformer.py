...
class Transformer3D(nn.Module):
    def __init__(self, img_dim, in_channels, embedding_dim, head_num, mlp_dim,
        ...
        self.n_patches = int(
            (img_dim[0] * img_dim[1] * img_dim[2]) / (patch_size ** 3))

        # create patch embeddings using convolutional layer
        self.patch_embeddings = nn.Conv3d(in_channels, embedding_dim,
                                          kernel_size=patch_size, stride=patch_size, bias=False)

        self.position_embeddings = AbsPositionalEncoding1D(
            self.n_patches, embedding_dim)
        self.dropout = nn.Dropout(0.1)

        self.transformer = TransformerEncoder(
            embedding_dim, head_num, mlp_dim, block_num, z_idx_list)

    def forward(self, x):
        embeddings = rearrange(self.patch_embeddings(x),
                               'b d x y z -> b (x y z) d')
