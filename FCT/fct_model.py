...
class Attention(nn.Module):
    def __init__(self,
        ...
        self.conv_q = nn.Conv2d(channels, channels, kernel_size,
                                stride_q, padding_q, bias=attention_bias, groups=channels)  # depth wise convolution
        # layer norm is calculated over the last D dimensions
        self.layernorm_q = nn.LayerNorm(channels, eps=1e-5)
        self.conv_k = nn.Conv2d(channels, channels, kernel_size,
                                stride_kv, stride_kv, bias=attention_bias, groups=channels)
...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(
            out_channels, out_channels, 3, 1, padding="same")
    ...
    def forward(self, x):
        ...
        x3 = x3.permute(0, 3, 1, 2)  # change back to x2 shape
...
class Wide_Focus(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv2 = nn.Conv2d(in_channels, out_channels,
                               3, 1, padding="same", dilation=2)

        # increase receptive fields
        self.conv3 = nn.Conv2d(in_channels, out_channels,
                               3, 1, padding="same", dilation=3)

        # no dilation to capture finer details
        self.conv4 = nn.Conv2d(in_channels, out_channels, 3, 1, padding="same")

    def forward(self, x):
        # cycle of conv (with / without dilation) -> relu -> dropout
        x1 = self.conv1(x)
        x1 = F.gelu(x1)
        x1 = F.dropout(x1, 0.1)
...
class Block_encoder_bottleneck(nn.Module):
    def __init__(self, blk, in_channels, out_channels, att_heads, dpr):
        ...
        if ((self.blk == "first") or (self.blk == "bottleneck")):
            # for first encoder block, in_channels=1, out_channels = filter[0] (8)
            self.layernorm = nn.LayerNorm(in_channels, eps=1e-5)
            self.conv1 = nn.Conv2d(
                in_channels, out_channels, 3, 1, padding="same")
...
class Block_decoder(nn.Module):
    def __init__(self, in_channels, out_channels, att_heads, dpr):
        ...
        self.conv1 = nn.Conv2d(in_channels, out_channels, 3, 1, padding="same")
        self.conv2 = nn.Conv2d(
            out_channels*2, out_channels, 3, 1, padding="same")
        self.conv3 = nn.Conv2d(
            out_channels, out_channels, 3, 1, padding="same")
        self.trans = Transformer(out_channels, att_heads, dpr)
...
class FCT(nn.Module):
    def __init__(self, num_class):
        ...
        self.block_6 = Block_decoder(
            filters[4], filters[5], att_heads[5], dpr[5])
        self.block_7 = Block_decoder(
            filters[5], filters[6], att_heads[6], dpr[6])
        self.block_8 = Block_decoder(
            filters[6], filters[7], att_heads[7], dpr[7])
        self.block_9 = Block_decoder(
            filters[7], filters[8], att_heads[8], dpr[8])

        self.ds7 = DS_out(filters[6], num_class)  # num class
        self.ds8 = DS_out(filters[7], num_class)  # num class
...
def get_fct(num_class):
    model = FCT(num_class=num_class)
